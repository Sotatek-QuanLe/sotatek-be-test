...
These parts **must be completed** for the assignment to be considered a pass:
...
- [ ] **Error handling** when external services fail/timeout

#### Code Quality (Main Evaluation Criteria)
...
### 1.2 SHOULD-HAVE (If time permits)

| Feature | Effort | Impact | Recommendation |
|---------|--------|--------|-------------|
| Integration tests | Medium | High | Write 1-2 basic tests |
| Swagger/OpenAPI docs | Low | Medium | Add springdoc-openapi |
| Basic logging | Low | Medium | Slf4j + structured log |

---

### 1.3 SHOULD NOT Deep Dive (Time Trap)

Areas that consume time where interviewers **DO NOT expect** deep dives within 4 hours:

| Feature | Reason to avoid deep dive |
|---------|-------------------------|
| Circuit Breaker (Resilience4j) | Listed as "Bonus" - nice to have |
| Retry with exponential backoff | Complex, bug-prone, time-consuming to test |
| Docker + docker-compose | Deployment concern only |
| Database migration (Flyway/Liquibase) | H2 in-memory is sufficient for demo |
| Comprehensive monitoring | Beyond 4-hour scope |
| Complex validation rules | Basic validation is sufficient |
| Caching layer | Over-engineering |
| Async processing | Not required |
...
| Reason | Explanation |
|--------|-------------|
| **Balance** | Balances clean code and delivery speed |
| **Testability** | Interface abstraction allows easy mocking of external services |
| **SOLID Compliance** | Ensures Dependency Inversion Principle (DIP) |
| **Pragmatic** | Avoids over-engineering; fits the 4-hour constraint |
| **Familiar** | Based on Spring Boot conventions, easy to read and maintain |
...
|------------------|--------|
| Tight coupling with HTTP client | Hard to mock in unit tests |
| Fails SOLID principles | Lacks evaluation points |
| Hard to swap implementation | Inflexible when changes are needed |
...
| Reason to Reject | Impact |
|------------------|--------|
| Setup time ~45+ mins | Consumes >15% total time |
| 8-10 packages, much boilerplate | Risk of not finishing |
| Over-engineering for this scope | Interviewer may rate as "not pragmatic" |
| Only suitable for loose deadlines | Mismatch with 4-hour constraint |
...
| **Testing** | 45 min | Unit tests for business logic |
...
The assignment is considered **successfully completed** when:

- [ ] 5 REST endpoints work correctly
- [ ] External services are mocked and errors are handled
- [ ] Code is clean, with clear layer separation
...
1. Delivering working software within constraints
2. Code structure reflecting architectural thinking
3. Pragmatic handling of edge cases
4. Understanding the trade-off between "perfect" and "done"
...
|---|------------|------|-----------|
| 1 | "Low traffic" | 🔴 **CRITICAL** | Cannot predict traffic spikes. Black Friday, marketing campaign, viral moments. Race conditions become real. |
...
Strategies for answering challenges about these weaknesses:
...
| "Race condition?" | "Yes, oversell risk" | "Assumed low traffic" | "Redis distributed lock or DB pessimistic lock" |
| "No circuit breaker?" | "Yes, cascade failure risk" | "Bonus feature" | "Resilience4j with fallback" |
...
> **Key insight**: Knowing the design weaknesses is more important than having a perfect design.
...
|-----------|---------------|------|----------------------|
| **CancelOrderRequest DTO** | Full DTO class with `{ "status": "CANCELLED" }` | 🟡 Overkill | Use `POST /api/orders/{id}/cancel` without body |
| **OrderItemResponse DTO** | Separate DTO class | 🟡 Extra files | Can embed in OrderResponse |
| **Interface for every Service** | OrderService interface + Impl | 🟢 OK but skippable | Direct implementation with `@Service` is reliable |
| **3 External Client interfaces** | MemberClient, ProductClient, PaymentClient | 🟡 Many files | Could merge into 1 ExternalServiceClient |
| **Detailed Error Codes** | Multiple custom exceptions | 🟡 Time consuming | Generic `BusinessException` with message is sufficient |

### 9.2 Reasonable Simplifications (Keep These)

| Simplification | Why It's Smart |
|----------------|----------------|
| ✅ **H2 in-memory** | Zero config, auto-create schema, sufficient for demo |
| ✅ **Mock services in code** | No need to set up WireMock server |
| ✅ **No authentication** | Scope assumes "already authenticated at gateway" |
| ✅ **Sync flow** | Async is complex, timeout handling is hard |
| ✅ **Offset pagination** | Simple, sufficient for demo data |
| ✅ **No refund logic** | Out of scope, acknowledgement is enough |
| ✅ **3 statuses only** | PENDING → CONFIRMED → CANCELLED is sufficient |

### 9.3 Mark Out-of-Scope Explicitly

Add clear comments/docs for things NOT done:
...
| 2nd | PUT /orders (cancel) | Medium | 3 endpoints still pass |
| 3rd | Pagination | Medium | Return all orders, note "pagination pending" |
| 4th | Some unit tests | Medium | 1-2 happy path tests are sufficient |
...
> **Bottom line**: A working application with 3 endpoints + clean code > A perfect design that doesn't compile.
